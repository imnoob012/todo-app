...
- 実務で求められる **認証・認可・ロール管理** を含む Web アプリケーションを一から設計・実装する経験を積みたかった
- 単純な CRUD にとどまらず、**多対多のリレーション管理**（TODO ⇄ メンバー）や**きめ細かな権限制御**を実装することで、実践的なスキルを身につけたかった
- チーム開発を意識した **レイヤードアーキテクチャ・DB マイグレーション・テスト** の導入により、保守性の高いコードの書き方を学びたかった
...
Docker がインストールされていれば、以下のコマンドだけで PostgreSQL を含む全環境が起動します。

```bash
docker compose up
```

http://localhost:8080/login でアプリケーションにアクセスできます。
